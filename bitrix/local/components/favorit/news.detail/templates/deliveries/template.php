...
<script type="text/javascript">
    let coords = [];
    <?foreach ($arResult['YMAP_COORDS'] as $key => $value)
    {?>
        coords.push(
        {
            coordinates: "<?=$value["props"]["POINT"]["VALUE"];?>",
            description: "<?=$value["props"]["DESC"]["VALUE"]?>",
            iconContent: "<?=$value["props"]["ICONCONTENT"]["VALUE"]?>"
        });
        <?
    }
    ?>
    ...
    {
        let cs = coords[i].coordinates.split(",");
        coords[i].coordinates = [parseFloat(cs[0]),parseFloat(cs[1])];
        console.log(typeof(coords[i].coordinates[0]));
    }
    const init = function () {
        ...
        for (var i = 0; i < coords.length; i++)
        {
            myCollection.add(new ymaps.GeoObject(
            {
                ...
                properties: {
                    description: coords[i].description,
                    iconContent: coords[i].iconContent
